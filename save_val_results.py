...
def save_validation_results(num_examples=5):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("Using device:", device)

    save_dir = "results/val_examples"
    ...
    with torch.no_grad():
        for i in range(num_examples):
            ...
            save_path = os.path.join(save_dir, f"val_result_{i+1}.png")
            plt.tight_layout()
...
if __name__ == "__main__":
    save_validation_results(num_examples=5)